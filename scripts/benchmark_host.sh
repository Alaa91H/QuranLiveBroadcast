#!/bin/bash
# ==============================================================================
# Quran Live Broadcast — Encode Self-Benchmark (first boot)
...
# Portable parse: last "fps=" token of the final progress line (no grep -P:
# busybox/Alpine lacks PCRE)
BENCH_FPS="$(awk '{for (i=1;i<=NF;i++) if ($i ~ /^fps=/) {split($i,a,"="); v=a[2]}} END{print v+0}' "$BLOG" 2>/dev/null || echo 0)"
BENCH_INT="${BENCH_FPS%.*}"
...
log "bench=${BENCH_FPS}fps eff=${EFF_FPS}fps steal=${STEAL_PCT}% mem=${MEM_MB}MB -> $BENCH_PROFILE"
